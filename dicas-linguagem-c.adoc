...
- declaração de uma variável: `float x, y;`
- chamada de uma função como printf e scanf: `printf("Bom dia");`
- atribuição de uma valor a uma variável: `x = y + 10;`
- interrupção de um laço de repetição com o comando break: `break;`
- separação da inicialização, condição e incremento em um laço de repetição for: `for(int i = 0; i < 10; i++)`
- retorno de um valor para uma função: `return 0;`
